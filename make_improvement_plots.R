...
y_improv_label = 'Fold Increase in Neutralisation GMT\n(variant-modified / ancestral-based)'
x_improv_label = 'SARS-CoV-2 strain tested in vitro'
x_matching_improv_label = 'Vaccine Immunogen vs Variant Tested'
...
vsv_improvement_plot_all_data_by_variant = ggplot(vsv_improvement_data_new, aes(x=VariantGroup,y=VSVimprovement,fill=VariantGroup))+
  ...
  scale_fill_manual(values=c('Ancestral'='grey98','Variant'='grey90'), guide='none')+
...
vsv_improvement_plot_split_by_valency = ggplot(filter(vsv_improvement_data_new,Variant!='Ancestral'), aes(x=Valency,y=VSVimprovement,fill=Valency))+
  geom_boxplot()+
...
vsv_improvement_plot_split_by_priorstatus = ggplot(filter(vsv_improvement_data_new,Variant!='Ancestral'), aes(x=PriorStatusGroup,y=VSVimprovement,fill=PriorStatusGroup))+
  geom_boxplot()+
...
vsv_improvement_plot_split_by_priordoses = ggplot(filter(vsv_improvement_data_new,Variant!='Ancestral'), aes(x=PriorDoses,y=VSVimprovement,fill=PriorDoses))+
  geom_boxplot()+
  geom_point(aes(x=PriorDoses,colour=Variant, shape = FirstAuthor),size=3,position = position_jitter(width=.1))+
  #stat_summary(aes(label=round(after_stat(y), 2), fontface='bold'),fun=geomean, geom="text",vjust=-1,position = position_dodge(0.9)) +
  #stat_compare_means(comparisons = list(c('2 Doses','3 Doses')),paired=F,method='t.test',label=c("p.signif"))+
  #stat_compare_means(paired=F,method='t.test',label.y = 2.75)+
  theme_classic()+
  scale_shape_manual(name = 'Reference',values=study_shapes, labels = study_shape_labels_2)+
